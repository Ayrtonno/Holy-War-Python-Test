...
def resolve_card_effect(engine: "GameEngine", player_idx: int, uid: str, target: str | None) -> str:
    ...
    if name_key == _norm("Cura Rapida"):
        ...
        for s_uid in engine.all_saints_on_field(player_idx):
            ...
            inst.current_faith = (inst.current_faith or 0) + amount
            healed += 1
        state.log(f"{player.name} usa Cura Rapida: +{amount} Fede a {healed} santi.")
        return "Cura Rapida risolta."

    if name_key == _norm("Ricerca Archeologica"):
        desired = _norm(target or "")
        chosen_uid = None
        if desired:
            for d_uid in player.deck:
                inst = state.instances[d_uid]
                if _norm(inst.definition.card_type) == "artefatto" and _norm(inst.definition.name) == desired:
                    chosen_uid = d_uid
                    break
            if chosen_uid is None:
                return "Ricerca Archeologica: artefatto richiesto non trovato nel reliquiario."
        else:
            # If no explicit target, auto-pick first artifact.
            for d_uid in player.deck:
                inst = state.instances[d_uid]
                if _norm(inst.definition.card_type) == "artefatto":
                    chosen_uid = d_uid
                    break
        if chosen_uid is None:
    ...
    if name_key == _norm("Monsone"):
        moved = 0
        while moved < 3 and player.hand:
            uid_h = player.hand.pop(0)
            player.graveyard.append(uid_h)
            moved += 1
        ...
        for owner_idx, f_uid in list(_iter_all_field_cards(engine)):
            ...
            if cv is None or cv > 8:
                continue
            engine.state.players[owner_idx].deck.insert(0, f_uid)
            engine._remove_from_board(engine.state.players[owner_idx], f_uid)
            returned += 1
        return "Monsone risolta."
