...
def on_activate(engine, player_idx: int, uid: str, target: str | None):
    ...
    if uid not in (player.attack + player.defense):
        return 'Tanngrisnir deve essere sul terreno per attivare l\'abilita.'
    ...
    for s_uid in engine.all_saints_on_field(player_idx):
        if _norm(engine.state.instances[s_uid].definition.name) == _norm('Thor'):
            ...
            break
    if thor_uid is None:
        return 'Serve Thor sul terreno per attivare Tanngrisnir.'
    ...
    engine.state.log(f"{player.name} sacrifica Tanngrisnir: Thor ottiene +4 Fede.")
    return 'Abilita Tanngrisnir risolta.'
