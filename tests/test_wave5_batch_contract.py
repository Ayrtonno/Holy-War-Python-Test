from __future__ import annotations

from holywar.effects.runtime import runtime_cards


WAVE5_CARDS = [
    "Ascensione",
    "Assalto Invernale",
    "Barriera Magica",
    "Barriera Magica Celestiale",
    "Bende Consacrate",
    "Contemplazione",
    "Cura",
    "Cura Totale",
    "Fioritura Primaverile",
    "Fujn'lyat",
    "Genesi: Compimento",
    "Giorno 1: Cieli e Terra",
    "Giorno 2: Cielo Terrestre",
    "Giorno 4: Stelle",
    "Legame Primordiale",
    "Medicina Egizia",
    "Mela del Peccato",
    "Memoria della Pietra",
    "Mummificazione",
    "Oggetti di Famiglia",
    "Passaggio all'Aldilà",
    "Perdono",
    "Ph'kdam",
    "Pietra Bianca",
    "Pioggia",
    "Pozione Celestiale",
    "Preghiera",
    "Preghiera: Fertilità",
    "Primo Sigillo",
    "Rinforzi",
    "Risveglio di Ph-Dak'Gaph",
    "Rito Funebre",
    "Ritorno Catastrofico",
    "Rituale Sepolcrale",
    "Sacri Vasi",
    "Sacrificio della Fertilità",
    "Sacrificio Naturale",
    "Settimo Sigillo",
    "Settimo Sigillo: Apocalisse",
    "Spore",
    "Ultima Offerta",
    "Veggenza",
    "Visioni Errate",
]


def _norm(name: str) -> str:
    import unicodedata

    value = unicodedata.normalize("NFKD", name)
    value = "".join(ch for ch in value if not unicodedata.combining(ch))
    return value.strip().lower()


def test_wave5_cards_are_scripted_with_on_play_actions() -> None:
    by_name = {k: v for k, v in runtime_cards._scripts.items()}  # noqa: SLF001 - contract test
    missing: list[str] = []
    bad_mode: list[str] = []
    no_actions: list[str] = []
    for name in WAVE5_CARDS:
        script = by_name.get(_norm(name))
        if script is None:
            missing.append(name)
            continue
        if str(script.on_play_mode).lower() != "scripted":
            bad_mode.append(name)
        if not script.on_play_actions:
            no_actions.append(name)
    assert missing == []
    assert bad_mode == []
    assert no_actions == []
