...
def make_def(name):
    return CardDefinition.from_dict({
        "name": name,
        "card_type": "santo",
        "crosses": "",
        "faith": 0,
...
# create 7 filler cards for deck+graveyard
for i in range(2, 9):
    uid = f"c{i:05d}"
    defn = make_def(f"Filler{i}")
    instances[uid] = CardInstance(uid=uid, definition=defn, owner=0, current_faith=defn.faith)

# place 5 in deck, 2 in graveyard
p0.deck = [f"c{i:05d}" for i in range(2, 7)]  # 2..6 => 5 cards
p0.graveyard = [f"c{i:05d}" for i in range(7, 9)]  # 7,8 => 2 cards
...
# Ensure registry has scripts (Hun-Came script) and refresh
runtime_cards.ensure_all_cards_migrated(engine)
runtime_cards.refresh_conditional_faith_bonuses(engine, 0)

print("strength", engine.get_effective_strength(hun_uid))
print("faith", engine.state.instances[hun_uid].current_faith)
